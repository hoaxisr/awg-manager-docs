...
**Kernel** — может давать более высокие скорости, но использует прослойку **OpkgTun** для связки с Entware (на OS 5.x). Используется не документированная возможность связки NDMS и kernel интерфейса, поэтому встроенные графики и механизмы роутера его не видят.
...
- **AmneziaWG 3.0/3.1** - новые параметры `ContentPaddingAddition`, `HeaderProtectionKey`, `DisableCookie` - необходимо учесть, что S4 должен быть больше 12, и MTU настроен в соотвествие с overhead настроенных параметров. 
...
В awg-manager встроена страница **Инструменты** (до 2.15.0 называлась «Диагностика») — откройте её и перейдите на вкладку **Проверки**.
...
![Системные тесты на странице Инструменты](/img/quickstart/diagnostics-tests.png)
...
- [Управление туннелями](../guide/tunnels/) — редактирование, расширенные AWG-параметры, маршрутизация
- [DNS-маршрутизация](../guide/dns-routing/) — пустить через туннель только определённые сайты
- [Мониторинг](../guide/monitoring/) — автоматический перезапуск при потере связности
- [Решение проблем](../troubleshooting/) — если туннель не подключается или работает нестабильно
