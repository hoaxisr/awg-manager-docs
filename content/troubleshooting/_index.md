...
Туннель поднялся, но awg-manager обнаружил проблему (упал процесс, не установилась связь и т.п.). Цвет LED — оранжевый, подпись на карточке — `Сломан` или `Восстановление (попытка N)` (если мониторинг ещё пытается перезапустить туннель).

Диагностика:

1. **Журнал туннеля** — страница **Диагностика** → вкладка **Журнал**, фильтр по имени туннеля
2. **Проверки туннеля** — страница **Диагностика** → вкладка **Проверки** → секция per-tunnel. Запустите **Проверка соединения**
...
- **Откройте Диагностика → Проверки**, запустите **Проверку IP**. Если IP не меняется при включённом туннеле — трафик не уходит через него
...
- Проверь, что компонент **Netfilter** установлен (см. [Sing-box Router → Если netfilter не установлен](../guide/singbox/router/#если-netfilter-не-установлен)).
- Проверь, что движок Sing-box Router включён (тумблер в подразделе **Движок**).
