...
### Как удалить интерфейсы OpkgTun если я не вижу их в разделе "Другие подключения"

Во-первых, не паникуйте. Все очень просто. Необходимо зайти в веб-конфигуратор роутера и нажать значок "Шестеренка". Далее в командной строке вводите команду `no interface OpkgTun` нажимаете кнопку `TAB` и выбираете какой из интерфейсов OpkgTun вы хотите удалить. После этого нажмите кнопку `Enter`.

>[!NOTE]
>Для того что узнать какой `OpkgTun` интерфейс соответствует какому-либо подключению, необходимо в том же разделе выполнить команду `show interface OpkgTun` нажать кнопку `TAB` и выбрать нужный интерфейс. В ответе вы увидите информацию о подключении и описание интерфейса.
...
>[!NOTE]
>При использовании singbox версии 1.14.0-awgm.16 и выше данная настройка в панели управления 3X-UI более не требуется.
