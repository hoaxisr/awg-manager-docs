...
- **Kernel-туннели** (интерфейсы OpkgTun) — до **7** штук; это прошивочное ограничение Keenetic, awg-manager на него повлиять не может.
- **NativeWG-туннели** — до 2.16.0 делили с kernel-туннелями общий пул из 7 слотов; с 2.16.0 у них собственный диапазон, и реальную ёмкость задают лимиты прошивки: до **100 интерфейсов Wireguard** в NDMS. На прошивках без нативного ASC туннели с обфускацией работают через модуль `awg_proxy` — таких одновременно запущенных может быть до **16** (лимит срабатывает при старте туннеля, с понятным сообщением; создавать интерфейсы он не мешает).
