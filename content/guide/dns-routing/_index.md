...
{{< pagemeta level="Базовый" os="5.x" ui="Маршрутизация → NDMS" >}}
...
Также см. [Sing-box Router](../singbox/router/) — третий движок маршрутизации, доступный при установленном sing-box на уровне Продвинутый.
...
- **Маршрут (порядок = приоритет)** — один или несколько целевых туннелей. Если туннелей несколько, порядок задаёт приоритет: awg-manager использует первый доступный, при его падении переключается на следующий (failover). Переключение между туннелями выполняет сам awg-manager — нативный механизм приоритетов NDMS в актуальных прошивках работает некорректно, поэтому awg-manager отслеживает доступность каждого туннеля и подменяет цель правила сам.
...
- **FQDN**: `example.com`, `dacha.example.com`
...
- Wildcards `*.youtube.com` — вместо этого укажите `rutube.ru` (NDMS сопоставляет по суффиксу, поддомены подпадают автоматически)
...
Если правило обслуживает большой часто обновляемый список (например, community листы собираемые по каком-то принципу), добавьте **URL подписки** — awg-manager будет периодически скачивать список и актуализировать правило. Данный функционал избыточен для случаев, когда вы добавляете сервис с заранее известными доменными именами. 

Формат — обычный текстовый файл со списком доменов по одному в строке. (RAW формат)
...
Вкладка **NDMS** доступна на **Keenetic OS 5.x и новее**. На более старых прошивках она не отображается — используйте [маршрутизацию по IP](../ip-routing/) или [HydraRoute Neo](../hr-neo/).
