...
{{< pagemeta level="Продвинутый" ui="Маршрутизация → HR NEO" requires="пакет HydraRoute Neo" >}}
...
Альтернативный движок продвинутой маршрутизации — [Sing-box Router](../singbox/router/) (доступен на уровне Продвинутый, работает через TPROXY/netfilter).
...
**Не нужно** добавлять устройства через веб-конфигуратор в политики, создаваемые HR NEO. Потому, что если трафик устройства попадет в фильтры (правила) указанные для политики отработают без этой настройки. В терминах HR политика это просто способ указать несколько интерфейсов в одном блоке.
{{< /callout >}}
...
{{< callout type="info" >}}
**Targets и правила можно упорядочивать** стрелками перемещения — порядок задаёт приоритет, первый в списке имеет наивысший. Если домен попадает в несколько целей или правил, сработает то, что выше.
{{< /callout >}}
...
С 2.15.0 гео-файлами можно управлять централизованно — на вкладке **Маршрутизация → Гео-данные**:

- **Автообновление гео-файлов** — единое расписание для всех geoip/geosite-файлов: каждые N часов (рекомендуется 6–24) или ежедневно в заданное время (по часам роутера).
- Кнопка **Запустить обновление сейчас** перекачивает все файлы разом.
- В строке каждого файла показывается время последнего обновления, а кнопка-иконка «Источник» открывает URL, с которого файл был загружен, с возможностью копирования.

![Вкладка Гео-данные](/img/guide/hr-neo/geodata.png)
