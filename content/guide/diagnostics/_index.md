...
{{< pagemeta level="Базовый" ui="Инструменты" >}}
...
Живой список **всех** соединений, проходящих через роутер: протокол, источник (с именем устройства, если оно известно NDMS), назначение (с подсказкой сервиса — YouTube, Discord, …), интерфейс, состояние и трафик.

В 2.16.0 страница переработана: сверху — кольцевые диаграммы распределения соединений по туннелям, назначениям и клиентам (клик по сегменту — быстрый фильтр), фильтры оформлены чипами, список можно сгруппировать по клиенту или по хосту назначения. Клик по строке открывает панель деталей соединения; отдельное соединение можно принудительно сбросить прямо отсюда. Плюс поиск по IP, порту или имени.
